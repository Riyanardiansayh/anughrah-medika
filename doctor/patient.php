        ...
        <tr class="menu-row">
            <td class="menu-btn menu-icon-appoinment">
                <a href="appointment.php" class="non-style-link-menu">
                    <div>
                        <p class="menu-text">Janji Saya</p>
                </a>
    </div>
    </td>
    </tr>
    ...
    <tr class="menu-row">
        <td class="menu-btn menu-icon-patient menu-active menu-icon-patient-active">
            <a href="patient.php" class="non-style-link-menu  non-style-link-menu-active">
                <div>
                    <p class="menu-text">Pasien Saya</p>
            </a></div>
        </td>
    </tr>
    <tr class="menu-row">
...
// =======================================
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        // 3B. JIKA TOMBOL FILTER DIKLIK
        if (isset($_POST['filter'])) {
            if ($_POST['shownly'] == "all") {
                // Tampilkan semua pasien
                $sqlmain = "SELECT * FROM patient";
                $selecttype = "All";
                $current = "Semua pasien";
            } else {
                // Tampilkan pasien yang punya jadwal appointment
                // bergabung dengan schedule dan appointment
                $sqlmain = "
                SELECT 
                    appointment.appoid, 
                    schedule.scheduleid, 
                    schedule.title, 
                    doctor.docname, 
                    patient.pid,       -- Pastikan ada kolom pid
                    patient.pname, 
                    patient.pemail,    -- Pastikan ada kolom pemail
                    patient.pnic,      -- Pastikan ada kolom pnic
                    patient.pdob,      -- Pastikan ada kolom pdob
                    patient.ptel,      -- Pastikan ada kolom ptel
                ...
                INNER JOIN patient 
                    ON patient.pname = appointment.pname
                INNER JOIN doctor 
    ...
    // 3C. JIKA TIDAK ADA POST, GUNAKAN QUERY DEFAULT
    if (empty($sqlmain)) {
        $sqlmain = "
        SELECT 
            appointment.appoid, 
            ...
            schedule.title, 
            doctor.docname, 
            patient.pid,       -- Pastikan kolom pid ada di table patient
            patient.pname, 
            patient.pemail,    -- Pastikan kolom pemail ada di table patient
            patient.pnic,      -- Pastikan kolom pnic ada di table patient
            patient.pdob,      -- Pastikan kolom pdob ada di table patient
            patient.ptel,      -- Pastikan kolom ptel ada di table patient
            schedule.scheduledate, 
            schedule.scheduletime, 
            appointment.apponum, 
            appointment.appodate 
...
// =======================================
                ...
                <td>
                    ...
                    <form action="" method="post" class="header-search">
                        ...
                        <input type="search" name="search12" class="input-text header-searchbar"
                            placeholder="Cari nama Pasien atau Email" list="patient">&nbsp;&nbsp;
                            ...
                            <tbody>
                                ...
                                if ($result->num_rows == 0) {
                                    ...
                                    for ($x = 0; $x < $result->num_rows; $x++) {
                                        $row = $result->fetch_assoc() ?? []; // Pastikan tidak null
                                        $pid = $row["pid"] ?? '';
                                        $name = $row["pname"] ?? '';
                                        $email = $row["pemail"] ?? '';
                                        $nic = $row["pnic"] ?? '';
    ...
    if ($_GET) {
        ...
        $result = $database->query($sqlmain);
        $row = $result->fetch_assoc() ?? []; // Jika query gagal, buat array kosong
